...
viewof matchMode = Inputs.radio(
  new Map([
    ["Fewest whole states", "smallest"],
    ["Closest numeric match", "closest"]
  ]),
  { value: "smallest" }
)
```
...
readout = {
  ...
  return html`<div>
    ...
    <p class="matched-list"><span class="matched-list-label">Matched states:</span>
...
treemapLeaves = [
  ...statePower.map(s => ({
    ...
    full: s.state_name,
    value: s.avg_gw,
    matched: matchedStateSet.has(s.state)
  })),
  ...selected.map(d => ({
    kind: "dc",
    key: `dc:${d.name}`,
...
treemap = {
  ...
  cell.append("rect")
      ...
      .attr("class", d => d.data.kind === "dc"
        ? "tm-rect tm-dc"
        : "tm-rect " + (d.data.matched ? "tm-matched" : "tm-unmatched"))
  ...
  cell.filter(d => fits(d, 40, 30))
    .append("text")
      .attr("class", d => "tm-value" + (d.data.kind !== "dc" && d.data.matched ? " tm-on-fill" : ""))
      .attr("x", 4)
      .attr("y", 26)
